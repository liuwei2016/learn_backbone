...
(function() {
    ...
    var Model = Backbone.Model = function(attributes, options) {
        ...
        if( defaults = getValue(this, 'defaults')) {
            ...
            /*如果Model在定义时设置了defaults默认数据,
             则初始化数据使用defaults与attributes参数合并后的数据
             (attributes中的数据会覆盖defaults中的同名数据)*/

            attributes = _.extend({}, defaults, attributes);
        }
        /* 显式指定模型所属的Collection对象(在调用Collection的add,
         push等将模型添加到集合中的方法时, 
         会自动设置模型所属的Collection对象)*/
        if(options && options.collection)
            this.collection = options.collection;
        /* attributes属性存储了当前模型的JSON对象化数据,
         创建模型时默认为空*/
        this.attributes = {};
        ...
        /*上面已经设置了初始化数据, changed, 
        _silent, _pending对象的状态可能已经
        发生变化, 这里重新进行初始化*/ 
        this.changed = {};
        this._silent = {};
        this._pending = {};
